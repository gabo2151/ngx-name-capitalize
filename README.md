# ngx-name-capitalize (Angular 14 – 15)

[![npm version](https://img.shields.io/npm/v/ngx-name-capitalize/legacy-v2)](https://www.npmjs.org/package/ngx-name-capitalize)
...
> **This is the `2.x` line, for Angular 14 and 15.** It is published under the
> `legacy-v2` npm tag, so `npm install ngx-name-capitalize` (which resolves `latest`)
...
| **2.x** | **14 – 15** | **`legacy-v2`** | **Maintenance — this branch** |
| 1.x | 12 – 13 | `legacy-v1` | Maintenance |

A major of this package means **one thing only: the minimum supported Angular version
went up**. It is not an API break. `2.x` and `3.x` carry the same library source; the
`3.x` line exists because it is built with the Angular 16 toolchain, not because
anything about the pipe changed.

Every release on this branch is linked against Angular 14 and 15 in CI
...
```bash
npm install ngx-name-capitalize@legacy-v2
```

> **Coming from 2.2.0 or earlier?** Those versions declared `peerDependencies` of
> `^14.0.0`, which npm reads as "14 and nothing else" — so installing them in an
> **Angular 15** project failed with `ERESOLVE`, even though this line has always
> supported 15. 2.3.0 fixes the range to `>=14.0.0 <16.0.0`. No code change was needed.
...
Both work on Angular 14 and 15 — pick whichever matches your app.
