...
[![Angular compatibility](https://github.com/gabo2151/ngx-name-capitalize/actions/workflows/ci.yml/badge.svg?branch=v2)](https://github.com/gabo2151/ngx-name-capitalize/actions/workflows/ci.yml?query=branch%3Av2)
...
**Complete and deliberately frozen, not abandoned.** This line exists to serve Angular 14
and 15, and that set will never grow — Angular 16 and above are served by `3.x`. So a
quiet commit history here is the design, not neglect.

It is still checked. Every week, CI links the **published artifact** against both Angular
majors this line supports, exactly the way your own build would, and runs the test suite
against the newest `name-capitalize`. The badge above is that job; it links to the runs.

<!-- compat:begin -->
Verified against Angular **14 and 15** — every major this line supports.
...
from an actual run, and CI fails if it ever stops matching what was verified.

If you are on Angular 16 or later, move to the `3.x` line — it gets the same fixes on a
modern build.
